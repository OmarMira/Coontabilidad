...
import { assignCustomerToARDDocument } from '@/database/modules/db-company';
                ...
                <div className="p-8 space-y-6">
                    ...
                    <div className="max-h-[300px] overflow-y-auto space-y-2 custom-scrollbar pr-2">
                        {filteredCustomers.map(c => (
                            ...
                            >
                                <div className="flex items-center gap-4">
                                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black ${selectedId === c.id ? 'bg-white text-indigo-600' : 'bg-white/10 text-slate-500'}`}>
                                        {c.name.charAt(0)}
                                    </div>
                                    <div className="text-left">
                                        <div className={`font-black text-sm uppercase ${selectedId === c.id ? 'text-white' : 'text-gray-200'}`}>{c.name}</div>
                                        <div className="text-[10px] font-bold text-slate-600 tracking-widest">ID: {c.id}</div>
                                    </div>
                                </div>
