        ...
        <Card className="bg-slate-900 border-white/5 text-white w-full">
            ...
            <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {roles.map(role => (
                        <div key={role.id} className="bg-white/10 border border-white/10 rounded-lg p-4 hover:border-blue-500 transition-colors cursor-pointer">
                            ...
                            <div className="mt-4 flex flex-wrap gap-2">
                                {role.permissions.map((p, i) => (
                                    <span key={i} className="text-xs bg-slate-900 px-2 py-1 rounded border border-white/10 font-mono text-slate-400">
                                        {p}
                                    </span>
                                ))}
                            </div>
