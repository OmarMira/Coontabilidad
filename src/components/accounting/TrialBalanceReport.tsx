        ...
        <div className="space-y-10 animate-in fade-in duration-500 pb-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-1 bg-slate-900/50 border border-slate-800/80 rounded-[2rem] shadow-2xl backdrop-blur-xl">
                <div className="p-6 bg-slate-950/80 rounded-[1.8rem] border border-slate-800/50 space-y-3">
                    <div className="flex items-center justify-between">
                        <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{t('accounting.trialBalance.openingBalance')}</span>
                ...
                <div className="p-6 bg-slate-950/40 rounded-[1.8rem] border border-slate-800/20 space-y-3">
                    <div className="flex items-center justify-between">
                        <span className="text-[10px] font-black text-emerald-500 uppercase tracking-widest">{t('accounting.trialBalance.debitFlow')}</span>
                ...
                <div className="p-6 bg-slate-950/40 rounded-[1.8rem] border border-slate-800/20 space-y-3">
                    <div className="flex items-center justify-between">
                        <span className="text-[10px] font-black text-rose-500 uppercase tracking-widest">{t('accounting.trialBalance.creditFlow')}</span>
                ...
                <div className={`p-6 rounded-[1.8rem] border-2 space-y-3 ${isBalanced ? 'bg-emerald-900/10 border-emerald-500/20' : 'bg-rose-900/10 border-rose-500/30'}`}>
                    <div className="flex items-center justify-between">
                        <span className="text-[10px] font-black uppercase tracking-widest">{t('accounting.trialBalance.integrity')}</span>
                        {isBalanced ? <CheckCircle className="w-4 h-4 text-emerald-500" /> : <ShieldAlert className="w-4 h-4 text-rose-500 animate-pulse" />}
                    </div>
                    <p className={`text-xl font-black ${isBalanced ? 'text-emerald-500' : 'text-rose-500'}`}>{isBalanced ? t('accounting.trialBalance.calibrated') : `${t('accounting.trialBalance.error')}: ` + formatCurrency(difference)}</p>
                </div>
            </div>
            ...
            <div className="flex flex-col xl:flex-row items-center justify-between gap-8 border-b border-slate-800 pb-10">
                <div className="flex items-center gap-5">
                    ...
                    <div>
                        <h2 className="text-4xl font-black text-white tracking-tighter uppercase leading-none">{t('accounting.trialBalance.title')}</h2>
                ...
                <div className="flex flex-wrap items-center gap-4">
                    ...
                    <div className="flex items-center gap-4 bg-slate-950 border border-slate-800 p-2 rounded-2xl shadow-inner">
                        ...
                        <div className="flex flex-col pr-4">
                            <span className="text-[8px] font-black text-slate-600 uppercase tracking-widest">{t('accounting.trialBalance.periodLabel')}</span>
                            <input
                                type="month"
                                value={period}
            ...
            <div className="rounded-[2.5rem] border border-slate-800 overflow-hidden bg-slate-950/20 backdrop-blur-2xl shadow-2xl">
                <table className="w-full text-sm text-left border-collapse">
                            ...
                            <tr>
                                <td colSpan={8} className="py-32 text-center">
                                    <div className="flex flex-col items-center gap-4">
                                        ...
                                        <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{t('accounting.trialBalance.syncing')}</p>
                                    </div>
                                </td>
                            </tr>
                        ) : data.length === 0 ? (
                            <tr>
                                <td colSpan={8} className="px-10 py-24 text-center">
                                    <div className="space-y-4 opacity-30">
                                        ...
                                        <p className="font-black text-xl text-slate-500 uppercase tracking-tighter">{t('accounting.trialBalance.noMovements')}</p>
                                        <div className="flex justify-center gap-4 pt-4 no-print">
                                            <button onClick={handleRepairAccounts} className="text-[9px] font-black text-slate-600 hover:text-white uppercase tracking-widest px-4 py-2 border border-slate-800 rounded-lg transition-all">{t('accounting.trialBalance.repairStructure')}</button>
                                            <button onClick={handleGenerateTestData} className="text-[9px] font-black text-slate-600 hover:text-white uppercase tracking-widest px-4 py-2 border border-slate-800 rounded-lg transition-all">{t('accounting.trialBalance.injectDemo')}</button>
                                        </div>
                                    </div>
                                </td>
                            </tr>
                    ...
                    <tfoot className="bg-slate-950/80 border-t-2 border-slate-800">
                        <tr>
                            <td colSpan={4} className="px-10 py-10 text-right font-black uppercase text-[10px] tracking-[0.3em] text-slate-600">{t('accounting.trialBalance.auditConsolidation')}</td>
                            <td className="px-10 py-10 text-right">
                                <p className="text-[9px] text-emerald-500 font-black uppercase tracking-widest mb-1.5">{t('accounting.trialBalance.debitConsumption')}</p>
                                <p className="font-mono text-2xl font-black text-emerald-400 tracking-tighter">
                                    {formatCurrency(totalDebit)}
                                </p>
                            </td>
                            <td className="px-10 py-10 text-right">
                                <p className="text-[9px] text-rose-500 font-black uppercase tracking-widest mb-1.5">{t('accounting.trialBalance.creditConsumption')}</p>
                            ...
                            <td colSpan={2} className="px-10 py-10 text-center">
                                <div className={`inline-flex flex-col items-center gap-1.5 px-10 py-5 rounded-[2rem] border-2 shadow-2xl transition-all ${isBalanced
                                    ...
                                    {!isBalanced && (
                                        <p className="text-[10px] font-black text-rose-500/80 uppercase tracking-widest">{t('accounting.trialBalance.diff')}: ${formatNumber(difference)}</p>
                                    )}
                                </div>
                            </td>
                        </tr>
                    ...
                    <div className="w-full max-w-6xl bg-slate-900/80 rounded-[3rem] border border-slate-800 shadow-[0_0_150px_rgba(59,130,246,0.1)] h-[85vh] flex flex-col overflow-hidden animate-in slide-in-from-bottom-8 duration-500">
                            ...
                            <div className="flex items-center gap-6">
                                ...
                                <div>
                                    ...
                                    <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.2em] mt-1 flex items-center gap-2">
                                        <FileText className="w-3.5 h-3.5" /> {t('accounting.trialBalance.registeredTrans')}
                                    </p>
                                </div>
                            </div>
                        ...
                        <div className="flex-1 overflow-auto px-4 custom-scrollbar">
                            <table className="w-full text-left">
                                ...
                                <tbody className="divide-y divide-slate-800/40">
                                    {movements.length === 0 ? (
                                        <tr><td colSpan={5} className="py-40 text-center text-slate-700 font-black uppercase tracking-widest italic opacity-30">{t('accounting.trialBalance.zeroMovements')}</td></tr>
                                    ) : (
                        ...
                        <footer className="px-12 py-10 bg-slate-950/80 border-t border-slate-800 flex justify-between items-center">
                            <div className="flex gap-4">
                                <div className="px-4 py-2 bg-slate-900 border border-slate-800 rounded-xl">
                                    <span className="text-[8px] font-black text-slate-600 uppercase tracking-widest block">{t('accounting.trialBalance.totalRecords')}</span>
                                    <span className="text-white font-black">{movements.length} {t('accounting.trialBalance.entries')}</span>
                                </div>
                            </div>
                            <div className="flex gap-12">
                                <div className="text-right">
                                    <p className="text-[9px] font-black text-emerald-500 uppercase tracking-[0.2em] mb-1">{t('accounting.trialBalance.closingDebit')}</p>
                                    <p className="text-3xl font-black text-emerald-400 tracking-tighter">{formatNumber(movements.reduce((a, c) => a + c.debit, 0))}</p>
                                </div>
                                <div className="text-right">
                                    <p className="text-[9px] font-black text-rose-500 uppercase tracking-[0.2em] mb-1">{t('accounting.trialBalance.closingCredit')}</p>
                                    <p className="text-3xl font-black text-rose-400 tracking-tighter">{formatNumber(movements.reduce((a, c) => a + c.credit, 0))}</p>
                                </div>
                            </div>
                        </footer>
                    </div>
                    ...
                    <div className="w-full max-w-2xl bg-slate-900 rounded-[3rem] border border-slate-800 shadow-2xl p-1 overflow-hidden animate-in zoom-in-95 duration-500">
                        <div className="bg-slate-950/50 p-12 space-y-10">
                            <header className="text-center space-y-4">
                                ...
                                <h3 className="text-3xl font-black text-white uppercase tracking-tighter">{t('accounting.trialBalance.netDiagnosis')}</h3>
                                <p className="text-slate-500 font-bold uppercase tracking-[0.2em] text-[10px]">{t('accounting.trialBalance.verification')}</p>
                            </header>

                            <div className="space-y-6">
                                {integrityResult.isValid ? (
                                    <div className="p-8 bg-emerald-500/5 rounded-3xl border border-emerald-500/10 text-center">
                                        <p className="font-black text-emerald-500 text-lg uppercase tracking-tight mb-2">{t('accounting.trialBalance.perfectSync')}</p>
