...
export const LedgerHub: React.FC<LedgerHubProps> = ({ chartOfAccounts = [], onNavigate }) => {
    ...
    // Debug logging
    useEffect(() => {
        console.log('LedgerHub Mounted', { activeTab, accountsCount: chartOfAccounts?.length });
    }, [activeTab, chartOfAccounts]);

    if (hasError) {
        return (
            <div className="p-8 bg-red-500/10 border border-red-500 rounded-xl text-red-500 text-center">
                <h3 className="font-bold text-xl">Error cargando módulo</h3>
                <Button onClick={() => setHasError(false)} className="mt-4">Reintentar</Button>
            </div>
        );
    }

    const tabs = [
        { id: 'summary', label: 'Resumen Diario', icon: PieChartIcon },
    ...
    try {
        ...
        console.error("Critical Render Error in LedgerHub:", err);
        setHasError(true);
        return null;
    }
};
