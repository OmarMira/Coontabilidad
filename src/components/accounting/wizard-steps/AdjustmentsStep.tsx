  ...
  const [checks, setChecks] = useState<ChecklistItem[]>([
    {
      ...
      label: t('adjustments.checks.depreciation'),
      status: 'pending',
      message: undefined,
      action: {
        label: t('adjustments.checks.calcDepreciation'),
        onClick: () => window.location.href = '/assets/fixed-assets'
      }
    },
    {
      id: 'adjustment-entries-recorded',
      label: t('adjustments.checks.entriesRecorded'),
      status: 'pending',
      message: undefined,
      action: {
        label: t('adjustments.checks.createEntry'),
        onClick: () => window.location.href = '/accounting/journal-entries'
      }
    },
    {
      id: 'accruals-recorded',
      label: t('adjustments.checks.accruals'),
      status: 'pending',
      message: undefined
    },
    {
      id: 'inventory-reconciled',
      label: t('adjustments.checks.inventory'),
      status: 'pending',
      message: undefined,
      action: {
        label: t('adjustments.checks.goToInventory'),
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="bg-purple-600/10 border-l-4 border-purple-500 rounded-xl p-5 flex items-start gap-4 shadow-lg shadow-purple-950/20">
        ...
        <p className="text-sm font-bold text-purple-200/80 leading-relaxed">
          <span className="text-white font-black uppercase tracking-tighter mr-2">{t('adjustments.phaseTitle')}:</span>
          {t('adjustments.phaseDesc')}
        </p>
      </div>
      ...
      <div className="bg-slate-900/50 rounded-2xl p-6 border border-slate-800 shadow-inner group">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h5 className="text-[10px] font-black text-slate-500 uppercase tracking-widest group-hover:text-slate-300 transition-colors">{t('adjustments.bestPractices')}</h5>
        </div>
        <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {t<string[]>('adjustments.tips').map((tip, i) => (
