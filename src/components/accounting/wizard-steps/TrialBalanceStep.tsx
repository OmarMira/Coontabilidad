  ...
  const [checks, setChecks] = useState<ChecklistItem[]>([
    {
      ...
      label: t('trialBalanceStep.checks.generated'),
      status: 'pending',
      message: undefined
    },
    {
      id: 'debits-equal-credits',
      label: t('trialBalanceStep.checks.balanced'),
      status: 'pending',
      message: undefined
    },
    {
      id: 'no-unbalanced-accounts',
      label: t('trialBalanceStep.checks.noUnbalanced'),
      status: 'pending',
      message: undefined
    },
    {
      id: 'all-accounts-classified',
      label: t('trialBalanceStep.checks.classified'),
      status: 'pending',
      message: undefined
    }
  ]);
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="bg-blue-600/10 border-l-4 border-blue-500 rounded-xl p-5 flex items-start gap-4">
        ...
        <p className="text-sm font-bold text-blue-200/80 leading-relaxed">
          <span className="text-white font-black uppercase tracking-tighter mr-2">{t('trialBalanceStep.phaseTitle')}:</span>
          {t('trialBalanceStep.phaseDesc')}
        </p>
      </div>
      ...
      <div className="bg-slate-900/50 rounded-2xl border border-slate-800 overflow-hidden shadow-2xl">
        <div className="bg-slate-950/50 px-6 py-4 border-b border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <h5 className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{t('trialBalanceStep.preview')}</h5>
          </div>
          <button
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-950/20 text-[10px] font-black text-slate-500 uppercase tracking-widest border-b border-slate-800">
                  <th className="text-left py-4 px-6">{t('trialBalanceStep.mainAccount')}</th>
                  <th className="text-right py-4 px-6">{t('trialBalance.debits')}</th>
                  <th className="text-right py-4 px-6">{t('trialBalance.credits')}</th>
                  <th className="text-right py-4 px-6">{t('trialBalanceStep.netPosition')}</th>
                </tr>
              </thead>
              ...
              <tfoot className="bg-slate-950/40">
                <tr className="font-black text-white border-t-2 border-slate-700">
                  <td className="py-5 px-6 uppercase tracking-widest text-[10px] text-slate-500">{t('trialBalanceStep.periodTotals')}</td>
      ...
      <div className="bg-slate-900/50 rounded-2xl p-6 border border-slate-800 shadow-inner group">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h5 className="text-[10px] font-black text-slate-500 uppercase tracking-widest group-hover:text-slate-300 transition-colors">{t('trialBalanceStep.controlPoints')}</h5>
        </div>
        <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {t<string[]>('trialBalanceStep.tips').map((tip, i) => (
