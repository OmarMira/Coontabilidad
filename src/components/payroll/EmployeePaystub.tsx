    ...
    return (
      <div className="p-6">
        <div className="bg-red-900/20 border-l-4 border-red-500 rounded-xl p-4 flex items-center gap-3">
          ...
          <p className="text-red-400 font-bold">Comprobante de pago no encontrado</p>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-slate-900/50 rounded-2xl shadow-2xl overflow-hidden border border-slate-800 print:bg-white print:text-black print:rounded-none print:shadow-none print:border-none">
        ...
        <div className="bg-slate-950/50 p-8 border-b border-slate-800 print:bg-white print:border-gray-200">
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              <h1 className="text-3xl font-black text-white tracking-tighter uppercase print:text-black">COMPROBANTE DE PAGO</h1>
              <div className="flex items-center gap-2 text-slate-400 font-bold text-sm print:text-gray-600">
                <Calendar className="w-4 h-4" />
                Periodo: {formatDate(payroll.pay_period_start)} - {formatDate(payroll.pay_period_end)}
              </div>
            </div>
            <div className="text-right">
              <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest print:text-gray-500">Fecha de Pago</p>
        ...
        <div className="p-8 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="bg-slate-950/30 p-6 rounded-xl border border-slate-800/50 print:bg-gray-50 print:border-gray-200">
              <h2 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4 flex items-center gap-2 print:text-gray-500">
                <User className="w-4 h-4 text-blue-500" />
                Información del Empleado
              </h2>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest print:text-gray-400">Nombre</p>
                  <p className="text-sm font-black text-white print:text-black">{employee.first_name} {employee.last_name}</p>
                </div>
                <div>
                  <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest print:text-gray-400">ID Empleado</p>
                  <p className="text-sm font-black text-white print:text-black">#{employee.id}</p>
                </div>
                {employee.ssn && (
                  <div>
                    <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest print:text-gray-400">SSN</p>
                    <p className="text-sm font-black text-white print:text-black">***-**-{employee.ssn.slice(-4)}</p>
                  </div>
                )}
                <div>
                  <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest print:text-gray-400">Departamento</p>
            ...
            <div className="bg-slate-950/30 p-6 rounded-xl border border-slate-800/50 print:bg-gray-50 print:border-gray-200">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest print:text-gray-400">Tipo de Pago</p>
                  <p className="text-sm font-black text-white capitalize print:text-black">{employee.pay_type === 'salaried' ? 'Salario Fijo' : 'Por Horas'}</p>
                </div>
                <div>
                  <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest print:text-gray-400">Tasa de Pago</p>
                  <p className="text-sm font-black text-white print:text-black">
                    {employee.pay_type === 'hourly'
                      ...
                      : `${formatCurrency(employee.salary || 0)}/año`
                    }
                  </p>
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
              <h2 className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-2">INGRESOS</h2>
              <div className="bg-slate-950/40 rounded-xl overflow-hidden border border-slate-800/50">
                <table className="w-full text-left border-collapse">
                  <thead className="bg-slate-950/60 border-b border-slate-800">
                    <tr>
                      <th className="px-4 py-2 text-[9px] font-black text-slate-500 uppercase">Concepto</th>
                      <th className="px-4 py-2 text-right text-[9px] font-black text-slate-500 uppercase">Monto</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-800/50">
                    <tr className="text-sm">
                      <td className="px-4 py-3 text-slate-400 font-bold">Pago Regular ({payroll.regular_hours}h @ {formatCurrency(payroll.hourly_rate || 0)})</td>
                    ...
                    {payroll.overtime_hours > 0 && (
                      <tr className="text-sm bg-blue-500/5">
                        <td className="px-4 py-3 text-blue-400 font-bold">Horas Extras ({payroll.overtime_hours}h @ {formatCurrency((payroll.hourly_rate || 0) * 1.5)})</td>
                        <td className="px-4 py-3 text-right font-black text-blue-400">{formatCurrency(payroll.overtime_pay)}</td>
                      </tr>
                    )}
                    {(payroll.bonuses || 0) > 0 && (
                      <tr className="text-sm">
                        <td className="px-4 py-3 text-slate-400 font-bold">Bonificaciones</td>
                        <td className="px-4 py-3 text-right font-black text-white">{formatCurrency(payroll.bonuses || 0)}</td>
                      </tr>
                    )}
                    {(payroll.commissions || 0) > 0 && (
                      <tr className="text-sm">
                        <td className="px-4 py-3 text-slate-400 font-bold">Comisiones</td>
                  ...
                  <tfoot className="bg-slate-950 border-t border-slate-800">
                    <tr className="text-sm">
                      <td className="px-4 py-3 font-black text-white uppercase tracking-wider">Total Bruto</td>
            ...
            {/* Deductions Table */}
            <div className="space-y-4">
              <h2 className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-2">DEDUCCIONES</h2>
              <div className="bg-slate-950/40 rounded-xl overflow-hidden border border-slate-800/50">
                <table className="w-full text-left border-collapse">
                  <thead className="bg-slate-950/60 border-b border-slate-800">
                    <tr>
                  ...
                  <tbody className="divide-y divide-slate-800/50">
                    ...
                    {(payroll.other_deductions || 0) > 0 && (
                      <tr className="text-sm text-red-400/80">
                        <td className="px-4 py-3 font-bold">Otras Deducciones</td>
                  ...
                  <tfoot className="bg-slate-950 border-t border-slate-800">
                    <tr className="text-sm">
                      <td className="px-4 py-3 font-black text-white uppercase tracking-wider">Total Deducciones</td>
          ...
          <div className="bg-emerald-500/10 border-2 border-emerald-500/20 rounded-2xl p-8 relative overflow-hidden group print:bg-gray-100 print:border-gray-300 print:border-dashed">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 relative z-10">
              <div className="text-center md:text-left">
                <p className="text-[10px] font-black text-emerald-500 uppercase tracking-widest mb-1">Total a Recibir</p>
                <h3 className="text-xl md:text-2xl font-black text-white uppercase print:text-black">NETO A PAGAR</h3>
              </div>
              <div className="text-center md:text-right">
                ...
                <p className="text-xs text-emerald-500/60 font-bold mt-1">Depósitos directos procesados</p>
              </div>
            </div>
          </div>
          ...
          <div className="border-t border-slate-800 pt-8 print:border-gray-200">
            <h2 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4 print:text-gray-500">RESUMEN ANUAL (YTD)</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-slate-950/30 p-4 rounded-xl border border-slate-800/50 print:bg-gray-50 print:border-gray-200">
                <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest mb-1">Bruto YTD</p>
                <p className="text-base font-black text-white print:text-black">{formatCurrency(employee.ytd_gross_pay || 0)}</p>
              </div>
              <div className="bg-slate-950/30 p-4 rounded-xl border border-slate-800/50 print:bg-gray-50 print:border-gray-200">
                <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest mb-1">Fed Tax YTD</p>
                <p className="text-base font-black text-red-400 print:text-black">{formatCurrency(employee.ytd_federal_tax || 0)}</p>
              </div>
              <div className="bg-slate-950/30 p-4 rounded-xl border border-slate-800/50 print:bg-gray-50 print:border-gray-200">
                <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest mb-1">FICA YTD</p>
                <p className="text-base font-black text-red-400 print:text-black">{formatCurrency(employee.ytd_fica || 0)}</p>
              </div>
              <div className="bg-slate-950/30 p-4 rounded-xl border border-slate-800/50 print:bg-gray-50 print:border-gray-200">
                <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest mb-1">Medicare YTD</p>
        ...
        <div className="bg-slate-950/50 p-6 border-t border-slate-800 text-center print:bg-white print:border-gray-200">
          <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">
            Este es un recibo de pago generado por computadora. No requiere firma.
          </p>
          <p className="text-[9px] text-slate-600 font-bold mt-1 uppercase tracking-wider">
            Para dudas sobre su pago, contacte al departamento de Nómina.
          </p>
        </div>
      </div>
    </div>
