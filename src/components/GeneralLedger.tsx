      ...
      <div className="bg-slate-900/40 border border-slate-800 rounded-3xl p-6 backdrop-blur-md shadow-xl">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">{t('ledger.searchAccount')}</label>
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">{t('ledger.structure')}</label>
            <select
              value={filterType}
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">{t('ledger.from')}</label>
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">{t('ledger.to')}</label>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-1 border border-slate-800 rounded-[2rem] bg-slate-950/30 overflow-hidden flex flex-col h-[700px]">
          <div className="px-6 py-5 bg-slate-900/50 border-b border-slate-800 flex items-center justify-between">
            <span className="text-[10px] font-black text-white uppercase tracking-widest">{t('ledger.masterPlan')}</span>
        ...
        <div className="lg:col-span-3">
          {selectedAccount ? (
              ...
              <div className="flex-1 overflow-auto bg-grid-slate-900/10">
                {isLoading ? (
                  <div className="py-40 flex flex-col items-center gap-5">
                    ...
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{t('ledger.querying')}</p>
                  </div>
                ) : accountLedger ? (
                  <>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 border-b border-slate-800 bg-slate-950/30">
                      <div className="p-8 border-r border-slate-800/50">
                        <span className="text-[9px] font-black text-slate-600 uppercase tracking-widest block mb-2">{t('ledger.opening')}</span>
                      ...
                      <div className="p-8 border-r border-slate-800/50 group hover:bg-emerald-500/5 transition-all">
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-[9px] font-black text-emerald-500 uppercase tracking-widest block">{t('ledger.debits')}</span>
                      ...
                      <div className="p-8 border-r border-slate-800/50 group hover:bg-rose-500/5 transition-all">
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-[9px] font-black text-rose-500 uppercase tracking-widest block">{t('ledger.credits')}</span>
                          <TrendingDown className="w-3 h-3 text-rose-500" />
                        </div>
                        <p className="text-lg font-black text-rose-400 font-mono">${accountLedger.total_credits.toFixed(2)}</p>
                      </div>
                      <div className="p-8">
                        <span className="text-[9px] font-black text-blue-500 uppercase tracking-widest block mb-2">{t('ledger.finalPointer')}</span>
            ...
            <div className="h-full min-h-[500px] border-2 border-dashed border-slate-800 rounded-[2.5rem] flex flex-col items-center justify-center p-12 text-center bg-slate-900/20">
              ...
              <h3 className="text-xl font-black text-slate-600 uppercase tracking-tighter">{t('ledger.selectNode')}</h3>
              <p className="text-[10px] font-black text-slate-700 uppercase tracking-[0.2em] mt-3 max-w-xs leading-loose">{t('ledger.selectNodeHelp')}</p>
            </div>
          )}
        </div>
      </div>
