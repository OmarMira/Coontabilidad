...
export const SalesInvoiceForm: React.FC<SalesInvoiceFormProps> = ({
    ...
    return (
        <div className="bg-white/10 rounded-lg p-6 border border-white/10 shadow-xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-900/50 p-4 rounded-lg">
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <select
                                ...
                                className="w-full pl-10 pr-4 py-2 bg-white/5 border border-white/10 rounded text-white focus:border-blue-500 outline-none"
                            >
                    ...
                    <div>
                        <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Jurisdicción Fiscal (Condado)</label>
                        <div className="relative">
                        ...
                        <p className="text-xs text-slate-600 mt-1">Determines Surtax Rate (e.g., Miami-Dade 1%)</p>
                    </div>
                </div>
                ...
                <div>
                    <div className="flex justify-between items-center mb-2">
                        <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Artículos</label>
                    ...
                    <div className="space-y-2">
                        {items.map((item, index) => (
                            <div key={index} className="grid grid-cols-12 gap-2 items-start bg-slate-900 p-2 rounded border border-white/5">
                                <div className="col-span-4">
                                    <select
                                        value={item.product_id}
                                        onChange={(e) => handleItemChange(index, 'product_id', e.target.value)}
                                        className="w-full bg-white/10 text-sm text-white border border-white/10 rounded px-2 py-1"
                                    >
                                    ...
                                    <input
                                        ...
                                        className="w-full mt-1 bg-transparent text-xs text-slate-500 border-none p-0 focus:ring-0"
                                    />
                                </div>
                                ...
                                <div className="col-span-1 text-right pt-2 text-sm font-mono text-slate-400">
                ...
                <div className="border-t border-white/10 pt-4 flex justify-between items-center">
                    <div className="text-right flex-1">
                        <span className="text-slate-500 mr-2">Subtotal Est.:</span>
                        <span className="text-xl font-black tracking-tight text-white">${subtotal.toFixed(2)}</span>
                        <p className="text-xs text-slate-600 mt-1">Impuesto y Total final calculados por el motor al enviar.</p>
                    </div>
                    <div className="ml-6">
                        <button
                            type="submit"
