                ...
                <div className="card-elite bg-gradient-to-br from-indigo-600/20 to-transparent border-indigo-500/20 !p-8 relative overflow-hidden">
                    <div className="relative z-10">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <span className="text-[10px] font-black text-indigo-300 uppercase tracking-widest">Pendiente por Conciliar</span>
                        </div>
                        <div className="text-4xl font-black text-white tabular-nums">{formatCurrency(pendingToCollect)}</div>
                        <p className="text-gray-500 text-xs mt-2 font-medium">Monto detectado en documentos procesados esperando cobro.</p>
                    </div>
                ...
                <div className="card-elite bg-gradient-to-br from-emerald-600/20 to-transparent border-emerald-500/20 !p-8 relative overflow-hidden">
                    <div className="relative z-10">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <span className="text-[10px] font-black text-emerald-300 uppercase tracking-widest">Recuperado (ARD)</span>
                        </div>
                        <div className="text-4xl font-black text-white tabular-nums">{formatCurrency(recoveredToday)}</div>
                        <p className="text-gray-500 text-xs mt-2 font-medium">Total de fondos convertidos exitosamente desde el módulo ARD.</p>
                    </div>
            ...
            <div className="card-elite !p-0 overflow-hidden border-white/5">
                <div className="p-6 border-b border-white/5 flex flex-col md:flex-row justify-between items-center gap-4 bg-white/[0.01]">
                    ...
                    <div className="flex items-center gap-2 text-gray-500">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-widest">Filtrando: Cobros Pendientes</span>
                    </div>
                </div>
                    ...
                    <table className="w-full text-left">
                        <thead className="bg-white/5">
                            <tr className="text-[10px] font-black text-gray-500 uppercase tracking-widest">
                                <th className="px-8 py-5">Origen ARD</th>
                                <th className="px-8 py-5">Detección OCR</th>
                                ...
                                <tr key={doc.id} className="hover:bg-white/[0.02] transition-colors">
                                    <td className="px-8 py-6">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <div className="text-[10px] font-black text-gray-600 uppercase tracking-widest">{doc.id}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-8 py-6">
                                        <div className="text-lg font-black text-white tabular-nums">{formatCurrency(doc.detected_amount || 0)}</div>
                                        <div className="text-[10px] font-bold text-gray-500">Fecha Doc: {doc.detected_date}</div>
                                    </td>
                                    <td className="px-8 py-6">
                                        {doc.status === 'converted' ? (
                                            <div className="flex items-center gap-2 text-emerald-400">
                                                <CheckCircle2 className="w-4 h-4" />
                                                <span className="text-[10px] font-black uppercase tracking-widest">COBRADO Y CONCILIADO</span>
                                            </div>
                                        ) : (
                                            <div className="flex items-center gap-2 text-sun-orange">
                                                <Clock className="w-4 h-4" />
                                                <span className="text-[10px] font-black uppercase tracking-widest">PENDIENTE DE PAGO</span>
                                            </div>
                                        )}
                                    </td>
                                    <td className="px-8 py-6 text-center">
                                        {doc.status === 'processed' ? (
                                            ...
                                            >
                                                REGISTRAR COBRO <ArrowUpRight className="w-3 h-3" />
                                            </button>
                                        ) : (
                                            <span className="text-gray-600 font-bold text-[10px] uppercase tracking-widest">Finalizado</span>
                                        )}
                                    </td>
                                </tr>
                            ))}
                            ...
                            {collectionDocs.length === 0 && (
                                <tr>
                                    <td colSpan={4} className="px-8 py-20 text-center">
                                        <p className="text-gray-600 font-black uppercase tracking-widest text-xs">No hay ítems pendientes de cobro financiero</p>
                                    </td>
                                </tr>
                            )}
                        </tbody>
