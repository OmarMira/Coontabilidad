...
export const ARDDocumentList: React.FC<ARDDocumentListProps> = ({ documents, onRefresh }) => {
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            case 'analyzing':
                return <span className="flex items-center gap-1.5 px-2 py-0.5 rounded-lg bg-indigo-500/10 text-indigo-400 text-[9px] font-black uppercase border border-indigo-500/20 animate-pulse">ANALIZANDO</span>;
            case 'processed':
                return <span className="flex items-center gap-1.5 px-2 py-0.5 rounded-lg bg-emerald-500/10 text-emerald-400 text-[9px] font-black uppercase border border-emerald-500/20">PROCESADO</span>;
            case 'converted':
                return <span className="flex items-center gap-1.5 px-2 py-0.5 rounded-lg bg-blue-500/10 text-blue-400 text-[9px] font-black uppercase border border-blue-500/20">CONVERTIDO</span>;
            default:
                return <span className="flex items-center gap-1.5 px-2 py-0.5 rounded-lg bg-gray-500/10 text-gray-400 text-[9px] font-black uppercase border border-gray-500/20">{status}</span>;
        }
    };
    ...
    return (
        <div className="card-elite !p-0 overflow-hidden">
            <div className="p-6 border-b border-white/5 flex justify-between items-center bg-white/[0.01]">
                <h3 className="text-table-header">Historial de Gestión GAR</h3>
                <span className="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em]">{documents.length} DOCUMENTOS</span>
            </div>
                ...
                <table className="w-full text-left">
                    <thead className="bg-white/5">
                        <tr className="text-[10px] font-black text-gray-500 uppercase tracking-widest">
                            ...
                            <th className="px-8 py-5 text-right">Monto Detectado</th>
                        ...
                        {documents.length === 0 && (
                            <tr>
                                <td colSpan={5} className="px-8 py-20 text-center">
                                    ...
                                    <p className="text-gray-500 font-bold uppercase tracking-widest text-xs">No hay registros en el gestor GAR</p>
                                </td>
                            </tr>
                        )}
                    </tbody>
