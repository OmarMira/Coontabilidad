        ...
        <div className="space-y-8 animate-in fade-in duration-700">
            <div className="flex items-center gap-6 p-10 bg-indigo-600/5 border border-indigo-500/20 rounded-[40px] relative overflow-hidden">
                ...
                <div className="relative z-10">
                    ...
                    <p className="text-gray-400 font-medium text-sm mt-1">Plan de despliegue para el motor de inteligencia documental más avanzado de Florida.</p>
                </div>
            ...
            <div className="relative pl-8 space-y-12 before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-[2px] before:bg-white/5">
                {steps.map((step, i) => (
                    <div key={i} className="relative group">
                        {/* Dot / Icon */}
                        <div className={`absolute -left-[41px] p-2 rounded-full border-4 border-slate-950 z-10 transition-transform duration-500 group-hover:scale-110 ${step.status === 'completed' ? 'bg-emerald-500 text-slate-950' : step.status === 'current' ? 'bg-indigo-600 text-white' : 'bg-slate-800 text-gray-500'}`}>
                        ...
                        <div className={`card-elite relative overflow-hidden transition-all duration-500 ${step.status === 'current' ? 'border-indigo-500/40 bg-indigo-500/[0.03]' : 'hover:border-white/10'}`}>
                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                                <div className="space-y-2">
                                    ...
                                    <p className="text-gray-500 text-sm font-medium leading-relaxed max-w-2xl">{step.description}</p>
                                </div>
