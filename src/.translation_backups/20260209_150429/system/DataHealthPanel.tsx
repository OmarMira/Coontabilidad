...
export const DataHealthPanel: React.FC = () => {
  ...
  const getStatusColor = (stat: HealthStatus) => {
    switch (stat) {
      case 'healthy':
        return { bg: 'bg-emerald-500/10', border: 'border-emerald-500/20', text: 'text-emerald-500', label: 'Saludable' };
      case 'warning':
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-slate-500 text-sm mt-1">
                {report
                  ? `Última verificación: ${new Date(report.timestamp).toLocaleString()}`
                  : 'Nunca verificado'}
              </p>
            </div>
          </div>
