...
/**
 * Crea una nueva categorÃ­a de activos
 */
...
export function disposeAsset(
  ...
  try {
    ...
    // TODO: Crear asiento contable de disposiciÃ³n
...
export const initDB = async (password?: string): Promise<any> => {
  ...
  try {
    logger.info('Database', 'init_start', 'Iniciando inicializaciÃ³n de base de datos SQLite');
...
'productos' as tipo,
  ...
  // Vista de resumen de impuestos Florida para IA - ORDEN NÂ°1
  db.run(`
...
VALUES(?, ?, ?, ?, ?, 1)
   ...
   * Cumple con el Paso 1 del Plan de ValidaciÃ³n Multi-Usuario
   */
...
// FunciÃ³n para generar hash de auditorÃ­a con chaining (SÃ­ncrona Real)
...
// Crear asiento contable automÃ¡tico
...
/**
 * Generar asiento automÃ¡tico para pago realizado (a proveedor)
 */
...
export const addPayment = (paymentData: Partial<SupplierPayment>, userId?: number): { success: boolean; message: string; paymentId?: number } => {
  ...
  try {
    ...
    // En createSchema linea 954 se crea supplier_payments. ConfÃ­o en que existe.
...
export const generateClosingEntry = (fromDate: string, toDate: string, userId?: number): { success: boolean; message: string; entryId?: number } => {
  ...
  try {
    ...
    // 1. Debitar Ingresos (Saldos acreedores -> DÃ©bito para cerrar)
    ...
    // 2. Acreditar Gastos (Saldos deudores -> CrÃ©dito para cerrar)
...
export function updateCompanyData(companyData: Partial<CompanyData>): { success: boolean; message: string; warnings?: string[] } {
  try {
    ...
    if (accountingDataCheck.hasData) {
      warnings.push('âš ï¸ ADVERTENCIA: Esta empresa tiene datos contables asociados');
      warnings.push(`ðŸ“Š ${accountingDataCheck.invoices} facturas, ${accountingDataCheck.customers} clientes, ${accountingDataCheck.suppliers} proveedores`);

      // Si se estÃ¡ cambiando el nombre de la empresa y hay datos contables
      if (companyData.company_name || companyData.legal_name) {
        warnings.push('ðŸ”„ Cambiar el nombre puede afectar reportes y documentos existentes');
        warnings.push('ðŸ“‹ Se recomienda crear un respaldo antes de continuar');
      }
    }
...
export function createProductCategory(categoryData: Omit<ProductCategory, 'id' | 'created_at' | 'updated_at'>, userId?: number): { success: boolean; message: string; id?: number } {
  try {
    logger.info('ProductCategories', 'create_start', 'Creando categorÃ­a de producto', categoryData);
  ...
  `, [categoryData.name.trim()]);
    ...
    logger.info('ProductCategories', 'create_success', 'CategorÃ­a creada', { id: categoryId, name: categoryData.name });

    return {
      success: true,
      message: `CategorÃ­a "${categoryData.name}" creada correctamente`,
      id: categoryId
    };

  } catch (error) {
    logger.error('ProductCategories', 'create_failed', 'Error al crear categorÃ­a', categoryData, error as Error);
    return {
      success: false,
      message: `Error al crear categorÃ­a: ${error instanceof Error ? error.message : 'Error desconocido'} `
    };
  }
}
...
/**
 * Crea un reporte DR-15 vacÃ­o para perÃ­odos sin datos
 */
...
export function createPaymentMethod(methodData: Omit<PaymentMethod, 'id' | 'created_at'>): { success: boolean; message: string; id?: number } {
  ...
  try {
    logger.info('PaymentMethods', 'create_start', 'Creando mÃ©todo de pago', methodData);
...
VALUES(?, ?, ?, ?, ?, ?)
    ...
    logger.info('PaymentMethods', 'create_success', 'MÃ©todo de pago creado correctamente', {
      id: newId,
      method_name: methodData.method_name
    });
    ...
    return {
      success: true,
      message: `MÃ©todo de pago "${methodData.method_name}" creado correctamente`,
      id: newId
    };

  } catch (error) {
    logger.error('PaymentMethods', 'create_failed', 'Error al crear mÃ©todo de pago', methodData, error as Error);
    return {
      success: false,
      message: `Error al crear mÃ©todo de pago: ${error instanceof Error ? error.message : 'Error desconocido'} `
    };
  }
}
...
export function canDeletePaymentMethod(id: number): { canDelete: boolean; reason?: string } {
  ...
  `, [id]);
    ...
    if (existingResult.length === 0 || existingResult[0].values.length === 0) {
      return { canDelete: false, reason: 'MÃ©todo de pago no encontrado' };
    }
    ...
    if (usageCount > 0) {
      return {
        canDelete: false,
        reason: `El mÃ©todo de pago estÃ¡ siendo usado en ${usageCount} transacciones.Solo se puede desactivar.`
      };
    }
    ...
    logger.error('PaymentMethods', 'can_delete_failed', 'Error al verificar si se puede eliminar mÃ©todo de pago', { id }, error as Error);
    return { canDelete: false, reason: 'Error al verificar el mÃ©todo de pago' };
  }
}
// ==========================================
