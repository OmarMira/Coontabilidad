...
const menuItems: MenuItem[] = [
  ...
  },
  {
    ...
    children: [
      ...
      { id: 'role-manager', label: 'Gesti├│n de Roles', icon: Shield },
  ...
  },
  {
    ...
    children: [
      ...
      { id: 'purchase-orders', label: '├ôrdenes de Compra', icon: ShoppingCart },
  ...
  },
  {
    ...
    icon: TrendingUp,
    children: [
      { id: 'ard-module', label: 'An├ílisis ARD', icon: ScanSearch },
  ...
  },
  {
    ...
    children: [
      ...
      { id: 'bank-reconciliation', label: 'Conciliaci├│n Bancaria', icon: FileText },
      { id: 'discrepancy-analysis', label: 'An├ílisis de Discrepancias', icon: BarChart3 },
      { id: 'bank-smart-import', label: 'Importaci├│n Bancaria IA', icon: Bot },
      { id: 'general-ledger', label: 'Libro Mayor', icon: FileText },
      { id: 'trial-balance', label: 'Balance de Comprobaci├│n', icon: BarChart3 },
      ...
      { id: 'aging-report', label: 'Reporte de Antig├╝edad', icon: Clock },
      { id: 'fixed-assets', label: 'Gesti├│n de Activos', icon: Package }, // MOVIDO AQU├ì
  ...
  // SECCIONES MOVIDAS FUERA DE CONTABILIDAD (SOLICITUD USUARIO)
  {
    ...
    icon: Users,
    children: [
      { id: 'employee-mgr', label: 'Gesti├│n de Empleados', icon: UserCheck },
      { id: 'payroll-process', label: 'Procesar N├│mina', icon: Calculator },
      { id: 'payroll-reports', label: 'Reportes de N├│mina', icon: BarChart3 }
    ]
  },
  {
    id: 'inventario',
    ...
    children: [
      ...
      { id: 'product-categories', label: 'Categor├¡as', icon: Package2 },
  ...
  },
  {
    ...
    icon: Receipt,
    children: [
      { id: 'tax-config', label: 'Configuraci├│n Fiscal', icon: Settings },
  ...
  },
  {
    ...
    icon: HelpCircle,
    children: [
      { id: 'accounting-diagnosis', label: 'Diagn├│stico Contable', icon: Activity },
      { id: 'journal-entry-test', label: 'Pruebas de Asientos', icon: FileText },
      { id: 'backups', label: 'Respaldos y Restauraci├│n', icon: HardDrive },
      { id: 'system-logs', label: 'Logs del Sistema', icon: Activity },
      { id: 'auditoria', label: 'Auditor├¡a de Transacciones', icon: Search },
      { id: 'verify', label: 'Verificaci├│n Iron Core', icon: Shield },
...
export const Sidebar: React.FC<SidebarProps> = ({ currentSection, onNavigate }) => {
  ...
  // Filtrar ├¡tems del men├║ seg├║n rol
  ...
  const toggleExpanded = (itemId: string) => {
    setExpandedItems(prev => {
      ...
      if (isAlreadyExpanded) {
        // Si ya est├í expandido, lo cerramos (todos quedan cerrados)
        return new Set();
      } else {
        // Si no est├í expandido, cerramos todos y abrimos solo este
  ...
  return (
    ...
    `}>
      ...
      {/* Perfil del Usuario y Bot├│n de Cerrar Sesi├│n */}
      <div className="p-4 border-t border-slate-900/50 mt-auto space-y-2">
        ...
        <button
          onClick={() => {
            if (confirm('┬┐Deseas cerrar la sesi├│n?')) {
              logout();
            }
          }}
        ...
        >
          ...
          {!isCollapsed && <span>CERRAR SESI├ôN</span>}
        </button>
      </div>

      {/* Bot├│n para colapsar/expandir mejorado */}
      <button
